...
RUN pip3 install --no-cache-dir 'python_openzwave==0.4.10' --install-option="--flavor=ozwdev"

# Use if we're using a different ozw version than hass
# replace component requirement 0.4.9 with 0.4.10
RUN sed -i 's/0.4.9/0.4.10/' /usr/src/app/homeassistant/components/zwave/__init__.py
